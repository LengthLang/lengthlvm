...
pub fn parse(file: &str) -> Result<Vec<Opcode>, &str> {
    ...
    if let Ok(code_string) = code {
        ...
        for (i, line) in lines.iter().enumerate() {
            ...
            match line.len() {
                ...
                14 => {
                    if i + 1 == lines.len() {
                        return Err("gotou must have a line under it");
                    }
                ...
                25 => {
                    if i + 1 == lines.len() {
                        return Err("push must have a line under it");
                    }
        ...
        Err("Could not open specified file")
    }
